...
- **Automatic MCP config resolution:** Auto-detects ~/.gemini.json
- **Better error handling:** Structured errors with actionable messages

**See `${extensionPath}/scripts/README.md` for full documentation.**
...
**At the beginning of EVERY session (Tech Lead or Subagent), if a Worktree path is provided in the context/request:**
...
- **If it doesn't exist**: Report error to Tech Lead (subagents) or create it (Tech Lead).

**If NOT in a worktree and NO path is provided (Tech Lead only):**
...
```bash
# Create worktree automatically (Tech Lead)
...
## Rules System (from everything-gemini-code)
