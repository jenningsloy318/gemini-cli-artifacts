...
<tools name="Search Tools">
  ...
  Exa (Web and Code Search — Supplementary): Web search via `${GEMINI_EXTENSION_ROOT}/scripts/exa/exa_search.sh`, code context search via `${GEMINI_EXTENSION_ROOT}/scripts/exa/exa_code.sh`.

  DeepWiki (GitHub Repo Documentation): Get repo docs structure via `${GEMINI_EXTENSION_ROOT}/scripts/deepwiki/deepwiki_structure.sh`, contents via `${GEMINI_EXTENSION_ROOT}/scripts/deepwiki/deepwiki_contents.sh`, ask questions via `${GEMINI_EXTENSION_ROOT}/scripts/deepwiki/deepwiki_ask.sh`.

  Context7 (Library Documentation): Resolve library ID via `${GEMINI_EXTENSION_ROOT}/scripts/context7/context7_resolve.sh`, get docs via `${GEMINI_EXTENSION_ROOT}/scripts/context7/context7_docs.sh`.

  GitHub (Code and Repo Search): Search code via `${GEMINI_EXTENSION_ROOT}/scripts/github/github_search_code.sh`, search repos via `${GEMINI_EXTENSION_ROOT}/scripts/github/github_search_repos.sh`, get file contents via `${GEMINI_EXTENSION_ROOT}/scripts/github/github_file_contents.sh`.
</tools>
