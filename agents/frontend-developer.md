...
<collaboration>
  Receive designs from ui-ux-designer. Get API contracts from backend-developer. Coordinate with qa-agent on test coverage. Triggered by Tech Lead directly (Domain-Aware Agent Routing) or dev-executor (fallback).
</collaboration>
