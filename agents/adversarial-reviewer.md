...
<output>
  <template>Load `${GEMINI_EXTENSION_ROOT}/templates/reference/adversarial-review-template.md` and fill in all placeholders.</template>
</output>

<process name="Iteration Behavior">
  PASS → proceed to Phase 10 (Documentation Update). CONTESTED → Tech Lead reviews findings, decides accept or loop back to Phase 8. REJECT → MUST loop back to Phase 8 with findings as input for dev-executor to fix.
</process>
