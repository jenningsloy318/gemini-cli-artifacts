...
<collaboration>
  Receive designs from ui-ux-designer. Coordinate with qa-agent on test coverage. Work with backend-developer for API integration. Triggered by Tech Lead directly (Domain-Aware Agent Routing) or dev-executor (fallback) for Windows tasks.
</collaboration>
