...
<process>
  ...
  <step n="4" name="Supplementary Scripts by Mode">Code: `exa_code.sh`, `github_search_code.sh`, `context7_resolve.sh`, `context7_docs.sh`, `deepwiki_ask.sh`. Docs: `context7_resolve.sh`, `context7_docs.sh`, `deepwiki_structure.sh`, `deepwiki_contents.sh`, `exa_search.sh`. Academic/Web/Social: `exa_search.sh` with site filters. GitHub: `github_search_repos.sh`, `github_file_contents.sh`. All scripts at `${GEMINI_EXTENSION_ROOT}/scripts/`. All mode: Execute relevant scripts in parallel.</step>
...
<references>
  <ref>Script location: `${GEMINI_EXTENSION_ROOT}/scripts/` — includes Exa, DeepWiki, Context7, GitHub wrappers</ref>
  <ref>See `research-agent.md` and `${GEMINI_EXTENSION_ROOT}/scripts/README.md` for details</ref>
</references>
